...
import com.payegis.applib.sdk.web.util.*;
...
public class IntegratedQueryServiceImpl implements IntegratedQueryService, CommConstants
{
    ...
    private @Value("${new.device.deadline}")
    Integer deadline;
    ...
    public Map<String, Object> getEnvironmentMonitorData(Integer userId, JSONObject jsonObject)
    {
        ...
        HashMap<String, String> queryBuildMap = new HashMap<>();
        ...
        {
            queryBuildMap.put("appId", jsonObject.getString("appId"));
        }
        else
        {
        ...
        {
            timeZone.add(DateUtil.getCurrentDateString(startTime) + " 00:00:00");
            timeZone.add(DateUtil.getCurrentDateString(startTime) + " 23:59:59");
        }
        else
        {
            ...
            {
                timeZone.add(DateUtil.getCurrentDateString(endTime) + " 23:59:59");
            }
        }
        if (StringUtils.isNotEmpty(appVersion))
        {
            queryBuildMap.put("appVersion", appVersion);
        }
        if (StringUtils.isNotEmpty(attribution))
        {
            queryBuildMap.put("attribution", attribution);
        }

        //组装风险类型数据
        if (StringUtils.isNotEmpty(riskType))
        {
            queryBuildMap = getRiskType(queryBuildMap, riskType);
        }

        sort.put("detectionTime", -1);
        Integer skipSize = (pageNow - 1) * 10;
        //不分页情况下查询改用户下的所有
    ...
    private HashMap<String, String> getRiskType(HashMap<String, String> queryBuildMap, String riskType)
    {
        switch (riskType)
        {
            case "isRoot":
                queryBuildMap.put("isRoot", true);
                return queryBuildMap;
            case "isEmulator":
                queryBuildMap.put("isEmulator", true);
                return queryBuildMap;
            case "modifierFrame":
            case "virusApps":
            case "piracyApps":
